...
namespace irr
{
	...
	enum EEVENT_TYPE
	{
		...
		//! A mouse input event.
		/** Mouse events are created by the irrlichtDevice and passed to IrrlichtDevice::postEventFromUser
		...
		*/
		...
		//! A key input event.
		/** Like mouse events, keyboard events are created by the irrlichtDevice and passed to
		IrrlichtDevice::postEventFromUser. They take the same path as mouse events. */
		EET_KEY_INPUT_EVENT,

		//! A joystick (joypad, gamepad) input event.
		/** Joystick events are created by polling all connected joysticks once per
		irrlichtDevice run() and then passing the events to IrrlichtDevice::postEventFromUser.
...
struct SEvent
{
	 ...
	 * each connected joystick once per run() of the irrlichtDevice. Joystick events will
	 * not be generated by default.  If joystick support is available for the
	 * active irrlichtDevice, _IRR_COMPILE_WITH_JOYSTICK_EVENTS_ is defined, and
...
struct SJoystickInfo
{
	...
	/** Note: with a Linux irrlichtDevice, the POV hat (if any) will use two axes. These
	 *  will be included in this count. */
	u32				Axes;

	//! An indication of whether the joystick has a POV hat.
	/** A Windows irrlichtDevice will identify the presence or absence or the POV hat.  A
	 *  Linux irrlichtDevice cannot, and will always return POV_HAT_UNKNOWN. */
	enum
	{
