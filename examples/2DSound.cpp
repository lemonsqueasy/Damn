...
#include <conio.h>
#else
#include "irrKlang/common/conio.h"
#endif
...
int main(int argc, const char** argv)
{
    ...
    engine->play2D(getMediaPath("getout.ogg"), true);
    ...
    do
    {
        ...
        // play a single sound
        engine->play2D(getMediaPath("bell.wav"));
    }
    while(getch() != 'q');
...
const char * getMediaPath(const char * fileName){
    ...
    unsigned tempLength = 1000;
    char * tempMediaPath = new char[tempLength];
    _NSGetExecutablePath(tempMediaPath, &tempLength);
    unsigned length = strlen(tempMediaPath) + strlen(addedPath) + strlen(fileName) + 1;
    char * mediaPath = new char[length];
    memcpy(mediaPath, tempMediaPath, strlen(tempMediaPath));
    delete[] tempMediaPath;
    tempMediaPath = nullptr;
    ...
    memcpy(mediaPath + strlen(mediaPath), fileName, strlen(fileName));
    return mediaPath;
